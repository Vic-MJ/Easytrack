...
-- Agregar columna ubicacion a reposition_contrast_fabrics si no existe
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'reposition_contrast_fabrics' AND column_name = 'ubicacion') THEN
        ALTER TABLE reposition_contrast_fabrics ADD COLUMN ubicacion TEXT NOT NULL DEFAULT '';
    END IF;
END
$$;
...
-- Tabla para contadores de folios de reposiciones
CREATE TABLE IF NOT EXISTS reposition_folio_counters (
    month_year VARCHAR(5) PRIMARY KEY, -- Formato "MM-YY"
