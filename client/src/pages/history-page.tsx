...
import { type Reposition, type Area, type RepositionHistory } from "@shared/schema";
...
export default function HistoryPage() {
  ...
  const { data: repositions = [], isLoading } = useQuery<any[]>({
  ...
  const getAreaBadgeColor = (area: Area) => {
    const colors: Record<Area, string> = {
      ...
      admin: "badge-admin",
      patronaje: "badge-corte",
      operaciones: "badge-corte",
      diseño: "badge-corte",
    };
  ...
  const isRepositionVisible = (reposition: any) => {
...
function RepositionHistoryContent({ repositionId }: { repositionId: number }) {
  const { data: history = [], isLoading } = useQuery<RepositionHistory[]>({
