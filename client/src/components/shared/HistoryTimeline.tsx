import { type RepositionHistory, type Area } from "@shared/schema";
...
interface HistoryTimelineProps {
  events: RepositionHistory[];
  title?: string;
  type?: 'order' | 'reposition';
  showDetailedInfo?: boolean;
}

export function HistoryTimeline({ events, title, type = 'reposition', showDetailedInfo = false }: HistoryTimelineProps) {
  ...
  const getTimeDifference = (currentDate: string | Date, previousDate?: string | Date) => {
  ...
  const getActionLabel = (action: string, fromArea?: string | null, toArea?: string | null) => {
    switch (action) {
      case 'created':
        return 'Reposición Creada';
      case 'transfer_accepted':
      ...
      case 'completed':
        return 'Reposición Finalizada';
      case 'paused':
        return 'Reposición Pausada';
      case 'resumed':
        return 'Reposición Reanudada';
      default:
        return action;
    }
  };
