...
const areas = [
  'patronaje', 'corte', 'bordado', 'ensamble', 'plancha', 'calidad', 'envios', 'operaciones', 'admin', 'almacen', 'diseño', 'maquilas'
];
...
const statusColors = {
  ...
  eliminado: 'bg-red-50 text-red-500 dark:bg-red-950/30 dark:text-red-500'
};
...
function RepositionSettingsDialog() {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetch('/api/settings/reposition_auto_approve')
        .then(res => res.json())
        .then(data => setAutoApprove(data.value === 'true'))
        .catch(err => console.error('Error fetching auto approve setting:', err));

      fetch('/api/settings/reposition_auto_complete')
        .then(res => res.json())
        .then(data => setAutoComplete(data.value === 'true'))
        .catch(err => console.error('Error fetching auto complete setting:', err));
    }
  }, [isOpen]);
  ...
  const handleToggle = async (key: string, value: boolean, setter: (val: boolean) => void) => {
    try {
      ...
      // Invalidate relevant queries if needed, though settings are mostly used on backend
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Configuración de Reposiciones</DialogTitle>
        </DialogHeader>
          ...
          <div className="flex items-center justify-between space-x-2">
            <Label htmlFor="auto-approve" className="flex flex-col space-y-1">
              ...
              <span className="font-normal text-xs text-muted-foreground">
                Las nuevas reposiciones se aprobarán automáticamente al crearse.
              </span>
            </Label>
            <Switch
              id="auto-approve"
              checked={autoApprove}
              onCheckedChange={(checked) => handleToggle('reposition_auto_approve', checked, setAutoApprove)}
            />
          </div>
          <div className="flex items-center justify-between space-x-2">
            <Label htmlFor="auto-complete" className="flex flex-col space-y-1">
              <span>Finalización Automática</span>
              <span className="font-normal text-xs text-muted-foreground">
                Las solicitudes de finalización se procesarán automáticamente sin requerir aprobación.
              </span>
            </Label>
            <Switch
              id="auto-complete"
              checked={autoComplete}
              onCheckedChange={(checked) => handleToggle('reposition_auto_complete', checked, setAutoComplete)}
            />
          </div>
        </div>
      </DialogContent>
...
export function RepositionList({ userArea }: { userArea: string }) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      Swal.fire({
        ...
        imageUrl: '/client/public/papelera.gif',
        imageWidth: 200,
        imageHeight: 200,
        imageAlt: 'Papelera',
        confirmButtonColor: '#8B5CF6'
  ...
  const getRepositionNextAreas = (currentArea: string) => {
    ...
    const allAreas = ['patronaje', 'corte', 'bordado', 'ensamble', 'plancha', 'calidad', 'operaciones', 'envios', 'diseño', 'almacen', 'maquilas'];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filtros */}
      <Card className="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 border-purple-200 dark:border-purple-700 transition-all duration-300">
        <CardHeader 
          ...
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-purple-800 dark:text-purple-300">
              ...
              {(searchTerm || statusFilter !== 'all' || urgencyFilter !== 'all' || filterAccident !== 'all' || typeFilter !== 'all' || dateRangeFilter !== 'all') && (
                <Badge className="bg-purple-600 hover:bg-purple-700 text-white ml-2 text-xs">
                  Filtros Activos
                </Badge>
              )}
            </CardTitle>
            <div className="flex items-center gap-2">
              {!isFiltersExpanded && (searchTerm || statusFilter !== 'all' || urgencyFilter !== 'all' || filterAccident !== 'all' || typeFilter !== 'all' || dateRangeFilter !== 'all') && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={(e) => {
                  ...
                  className="h-7 text-xs bg-white dark:bg-slate-800 text-purple-700 hover:text-purple-800 dark:text-purple-300 border-purple-200 dark:border-purple-800"
                >
                  Limpiar filtros
                </Button>
              )}
              <Button 
                variant="ghost" 
                size="sm" 
                className="text-purple-800 dark:text-purple-300 hover:bg-purple-100/30 dark:hover:bg-purple-800/30 h-8 w-8 p-0"
              >
                {isFiltersExpanded ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
              </Button>
            </div>
          </div>
        </CardHeader>
      ...
      <div className="grid gap-4">
        {repositions.length === 0 ? (
          ...
          filteredRepositions.map((reposition: Reposition) => (
            <Card key={reposition.id} className={cn(
              "overflow-hidden border-2 transition-all duration-300 hover:shadow-lg hover:border-purple-300 dark:hover:border-purple-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm",
              ...
              <CardContent className="p-5">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  ...
                  <div className="space-y-3 flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <div className="flex flex-wrap gap-1.5 ml-2">
                        <Badge className={cn("px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider", statusColors[reposition.status])}>
                          {reposition.status}
                        </Badge>
                        <Badge className={cn("px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider", urgencyColors[reposition.urgencia])}>
                          {reposition.urgencia}
                        </Badge>
                        <Badge variant="outline" className="px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-400">
                          {reposition.type}
                        </Badge>
                      </div>
                    </div>
                    ...
                    <div className="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
                      <span className="bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-md text-slate-800 dark:text-slate-200 font-semibold">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-gray-500 dark:text-gray-400 pt-2 border-t border-slate-100 dark:border-slate-800/80">
                      <div className="flex items-center gap-1.5">
                        <span className="font-semibold text-gray-400 dark:text-gray-500">Origen:</span>
                        <span className="bg-purple-50 dark:bg-purple-950/20 text-purple-700 dark:text-purple-300 px-1.5 py-0.5 rounded font-medium">
                          {getAreaDisplayName(reposition.solicitanteArea)}
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="font-semibold text-gray-400 dark:text-gray-500">Área Actual:</span>
                        <span className="bg-blue-50 dark:bg-blue-950/20 text-blue-700 dark:text-blue-300 px-1.5 py-0.5 rounded font-medium flex items-center gap-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2 lg:justify-end w-full lg:w-auto">
                    <Button
                      ...
                      className="bg-white dark:bg-slate-800 hover:bg-slate-50 border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-200"
                    >
                ...
                {/* Manual time form at full width below if active */}
                {manualTimes[reposition.id] && (
                  <div className="space-y-3 mt-4 p-4 border rounded-xl bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-800">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* Fecha y hora de inicio */}
                      <div className="space-y-2">
                        <Label htmlFor={`start-date-${reposition.id}`} className="text-sm font-semibold text-slate-700 dark:text-slate-300">Fecha de inicio:</Label>
                      ...
                      {/* Fecha y hora de fin */}
                      <div className="space-y-2">
                        <Label htmlFor={`end-date-${reposition.id}`} className="text-sm font-semibold text-slate-700 dark:text-slate-300">Fecha de fin:</Label>
                        <div className="grid grid-cols-2 gap-2">
                          <Popover>
                            <PopoverTrigger asChild>
                              <Button
