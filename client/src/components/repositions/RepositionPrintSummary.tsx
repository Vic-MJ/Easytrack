...
export function RepositionPrintSummary({ repositionId, onClose }: RepositionPrintSummaryProps) {
  ...
  const { data: pieces = [], isLoading: isLoadingPieces } = useQuery<any[]>({
  ...
  const { data: productos = [], isLoading: isLoadingProducts } = useQuery<any[]>({
  ...
  if (isLoading || isLoadingPieces || isLoadingProducts) {
    return (
  ...
  const totalCantidad = pieces.reduce((total: number, piece: any) => {
    ...
    return total + (piece.unit === 'pares' ? cantidad * 2 : cantidad);
  }, 0);
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-2 gap-2 print:gap-1 print:grid-cols-4">
                          ...
                          <div>
                            ...
                            <div className="text-sm border-b border-dotted border-gray-400 min-h-5 pt-1 print:text-xs print:min-h-3">
                              {productos.length > 1 ? 'Varios' : reposition.modeloPrenda}
                            </div>
                          </div>
                          <div>
                            <span className="font-semibold text-xs print:text-xs">Color:</span>
                            <div className="text-sm border-b border-dotted border-gray-400 min-h-5 pt-1 print:text-xs print:min-h-3">
                              {productos.length > 1 ? 'Varios' : reposition.color}
                            </div>
                          </div>
                          <div>
                        ...
                        {pieces.length > 0 && (
                          <div className="print:mb-1">
                            <span className="font-semibold text-[11px] print:text-[11px] text-gray-700">Detalle de Piezas por Producto:</span>
                            <div className="mt-1 space-y-2">
                              {productos.length > 0 ? (
                                productos.map((producto: any, prodIdx: number) => {
                                  const productPieces = pieces.filter((p: any) => {
                                    ...
                                    const startIndex = prodIdx * piecesPerProduct;
                                  ...
                                  return (
                                    <div key={producto.id || prodIdx} className="border border-gray-300 p-2 rounded print:p-1.5 bg-gray-50/50">
                                      <div className="font-bold text-xs text-purple-800 mb-1.5 print:text-[10px] flex justify-between">
                                        <span>PROD {prodIdx + 1}: {producto.modeloPrenda} • {producto.tela} • {producto.color} ({producto.tipoPieza})</span>
                                        <span className="text-slate-600 font-semibold">{productTotal} pz</span>
                                      </div>
                                      <div className="text-xs grid grid-cols-4 gap-1.5 print:grid-cols-8 print:text-xs">
                                        {productPieces.map((piece: any) => {
                                          const cantidad = typeof piece.cantidad === 'number' ? piece.cantidad : parseInt(piece.cantidad) || 0;
                                          return (
                                            <div key={piece.id} className="text-center border border-gray-300 p-1 rounded print:p-0.5 bg-white shadow-sm">
                                              <div className="font-extrabold print:text-[10px] text-gray-800">{piece.talla}</div>
                                              <div className="text-[10px] text-gray-600">
                                                {piece.unit === 'pares' ? `${cantidad} par` : `${cantidad} pz`}
                                                {piece.unit === 'pares' && <span className="text-[8px] text-gray-400 block">({cantidad * 2} pz)</span>}
                                              </div>
                                            </div>
                                          );
                                        })}
                                ...
                                <div className="text-xs grid grid-cols-4 gap-1 print:grid-cols-8 print:text-xs">
